...
FUNCTION pasquetta(pAnno NUMBER) RETURN VARCHAR2;
...
FUNCTION pasquetta(pAnno NUMBER) RETURN VARCHAR2 IS
...
begin
    ...
    /*G := mod(pAnno,19);
  C := trunc(pAnno/100);
  H := mod((C-trunc(C/4)-trunc((8*C+13)/25)+19*G+15),30);
  I := H-trunc((H/28)*(1-(H/28)*(29/(H+1))*((21-G)/11)));
  J := mod((pAnno+round(pAnno/4)+I+2-C+round(C/4)),7);
  L := I-J;
  mL := 3+(round((L+40)/44));
  mI := trunc((3+(L+40)/44),0);*/

    G := mod(pAnno,19);
  C := trunc(pAnno/100);
  H := mod((C-trunc(C/4)-trunc((8*C+13)/25)+19*G+15),30);
  I := H-trunc((H/28)*(1-(H/28)*(29/(H+1))*((21-G)/11)));
  J := mod(trunc((pAnno+(pAnno/4)+I+2-C+(C/4))),7);
  L := I-J;
  ...
  mI := trunc((3+(L+40)/44),0);
    ...
    gL := L+28-31*(trunc(mese/4,0));
    gI := trunc(L+28-31*(trunc(mese/4,0)),0);
    if(gL<gI) then
      giorno := gI-1;
    else
      giorno := gI;
	...
	pasqua := to_date(gg||'/'||mm||'/'||pAnno, 'dd/mm/yyyy');
	pasquetta := pasqua + 1;
	return to_char(pasquetta,'dd/mm/yyyy');
...
BEGIN
  ...
  if pDataFine < dtElab then
    wr2('La data di fine del nuovo periodo deve essere futura');
    raise_application_error(-20103, 'La data di fine del nuovo periodo deve essere futura');
  end if;
...
begin
  ...
  for periodo in cur_periodi_senza_giorno loop
    ...
    while trunc(giornoData) <= trunc(periodo.dt_fine) loop
      ...
      dPasquetta := nvl(to_date(pasquetta(anno), 'dd/mm/yyyy'), giornoData+1);
      ...
      if not ( (mese = 12 And (giorno = 8 Or giorno = 25 Or giorno = 26)) Or
         ...
         trunc(giornoData) <> trunc(dPasquetta) then
